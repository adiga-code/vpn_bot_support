...
### Пер-сервисное берётся из базы, в окружении — только общее

В окружении контейнера n8n живут три значения, и они не меняются при
добавлении ВПН-а:
...
Читаются как `{{ $env.ИМЯ }}`, а не `$vars`: раздел Settings → Variables есть
только на платных редакциях n8n, в self-hosted community его нет, и `$vars`
молча даёт `undefined`. Нужен `N8N_BLOCK_ENV_ACCESS_IN_NODE=false`, а в queue
mode — во всех трёх контейнерах (`n8n`, `worker`, `webhook`): выражения
считает воркер.
...
воркфлоу со своими credentials и своим `TG_TOKEN` в окружении.

...
```json
{
  "type": "ai_response",
  "dialog_id": "42",
  "message": "Передаю вас оператору, он скоро ответит.",
  "handoff": true,
  "handoff_reason": "клиент просит живого оператора"
}
```

> Основной сигнал автопередачи — явный флаг `handoff: true` (n8n-агент возвращает
> структуру `{answer, handoff, reason}`, готовый воркфлоу — `n8n_ai_agent.json`).
> `handoff_reason` показывается операторам в системном сообщении диалога.
> Legacy-вариант — токен `[HANDOFF]` в тексте `message` — по-прежнему
> распознаётся и вырезается.
>

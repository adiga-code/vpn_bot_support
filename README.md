...
1. **Панель → Настройки → Сервисы → Добавить сервис.** Слаг попадёт в ключи
   Redis, имя коллекции Qdrant (`kb_<слаг>`) и префикс идентификаторов диалогов
   (`<слаг>_`); после создания он не меняется.
2. **n8n: воркфлоу ИИ-агента.** Скопируйте `n8n_ai_agent.json`, подключите
   Telegram-триггер своего бота и передайте в под-воркфлоу входной параметр
   `service` со слагом. Ключи Redis (`vpn_bot:<слаг>:ai_settings`,
   `chat_history:<слаг>:<dialog_id>`) и поле `service` в исходящем сообщении
   RabbitMQ подставляются сами. **Вручную сменить нужно только коллекцию
   Qdrant** в ноде «Qdrant Vector Store» — на ту, что показана в таблице
   сервисов.
3. **n8n: исходящий роутер.** Добавьте переменную
   `TELEGRAM_BOT_TOKEN_<СЛАГ_ЗАГЛАВНЫМИ>` с токеном бота — роутер выбирает её по
   полю `service`. Для уведомлений операторов в Telegram-ноде креденшл выбирается
   вручную (n8n не умеет подставлять креденшлы выражением) — если сервисам нужны
   разные группы, скопируйте роутер целиком и укажите его адрес в поле «Вебхук
   n8n» у сервиса.
4. **Флаги операторов** — «Настройки → Операторы».
5. **База знаний и промпт** — загрузите документы и задайте промпт; они
   пер-сервисные, вкладки настроек показывают плашку с текущим ВПН-ом.
...
| POST | `/api/services` | Создать сервис (админ) |
| PUT | `/api/services/{id}` | Обновить: название, цвет, вебхук, активность (админ) |

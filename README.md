...
| `tcp` | серверы | доступность по TCP-порту + пинг |
| `http` | серверы | health-эндпоинт с JSON `{"load": …, "uptime": …}` |
| `mock_bot` | боты | МОК: выдуманные боты сервиса |
| `telegram` | боты | Bot API: `getMe` + `getWebhookInfo` (очередь апдейтов, ошибки вебхука) |
...
Токены живут в n8n и в панели не хранятся, поэтому `telegram`-провайдер
работает, только если токен сознательно продублировать в `config.bots[].token`
настройки мониторинга. Без него бот показывается как `unknown` с пояснением.
